...
TupleOperation::TupleOperation(string myrelation, int index_memblock_start, int size)
{
...
vector<Tuple> TupleOperation::getTuples(string table_name, int index_memblock_end)
{
...
vector<Tuple> TupleOperation::getUniqueTuples(string table_name, int index_memblock_end)
{
	...
	if(relation_pointer == NULL)
		throw string("Given Relation ").append(table_name).append(" doesn't exist");
	int num_blocks = relation_pointer->getNumOfBlocks();
	if(num_blocks > NUM_OF_BLOCKS_IN_MEMORY - 1)
	{
...
vector<Tuple> TupleOperation::getTuples(bool unique, string table_name, int index_memblock_end, vector<MyColumn> order_by)
{
...
vector<Tuple> TupleOperation::Ordering_Relation(vector<Tuple> tup, string table_name, vector<MyColumn> order_by)
{
		...
		{
			sort(tup.begin(), tup.end(), TupleProcessor(column));
		}
		else
		{
			Schema schema_obj = schema_manager.getSchema(table_name);
			string tempName = TableProcessor::rand_table();
...
vector<Tuple> TupleOperation::getTuples(ProjectOP project_op, SelectOP &select_op, string table_name, int index_memblock_end, string temp_relation)
{
...
vector<Tuple> TupleOperation::GetUniqueTuplesCorrected(bool isUnique, ProjectOP project_op, SelectOP& select_op, string table_name, int index_memblock_end, string temp_relation, vector<MyColumn> order_by)
{
		...
		if(isUnique)
		{
				...
				{
					string table = TableProcessor::rand_table();
		...
		if(order_by.size() != 0)
		{
				...
				if(memory_block_size < NUM_OF_BLOCKS_IN_MEMORY)
				{
					sort(tuples_displayed.begin(), tuples_displayed.end(), TupleProcessor(orderby));
				}
				else
				{
					string tblname = TableProcessor::rand_table();
...
vector<Tuple> TupleOperation::getTuples(bool isUnique, ProjectOP project_op, SelectOP& select_op, string table_name, int index_memblock_end, string temp_relation, vector<MyColumn> order_by)
{
	
	return GetUniqueTuplesCorrected(isUnique, project_op, select_op, table_name, 0, temp_relation, order_by);
}

vector<Tuple> TupleOperation::GetSelectTuples(string table_name, int index_memblock_end, SelectOP& select_op)
{
...
vector<Tuple> TupleOperation::GetSelectTuples(bool isUnique, string table_name, int index_memblock_end, SelectOP& select_op, vector<MyColumn> order_by)
{
	...
	if(isUnique)
	{
		...
		{
			tempTable = TableProcessor::rand_table();
...
vector<Tuple> TupleOperation::GetProd(string tbl1, string tbl2, string tempTable, bool saveToDB)
{
	...
	while(1)
	{
		...
		relation_bulk->getBlocks(start_Index, 1, block_toGet); 
		vector<Tuple> smaller_rel_tuples = mem.getTuples(1, block_toGet); 
		vector<Tuple>::iterator small_myiterator;
		int blocks_to_read = big_size;
		int index = 0;
		while(blocks_to_read > 0) 
		{
			relation_singleFetch->getBlock(index, 0); 
	...
	if(saveToDB)
	{
		SaveTupleTodisk(new_relationPointer, join_tuples, 0);
	}
	return join_tuples;
}
...
RelationSender TupleOperation::ExecuteProjectOP(ProjectOP project_op, JOINOP join_op,
					RelationSender sendRelation, bool saved)
{
	
	RelationSender return_relation;
...
string TupleOperation::Fetch_Table_Name(string column_name)
{
	...
	for(table_iterator = VariablesFromTbl.begin(); table_iterator != VariablesFromTbl.end(); table_iterator++)
	{
		...
		if(schema_t.fieldNameExists(column_name))
		{
			table_name = *table_iterator;
			count++;
		}
...
vector<Tuple> TupleOperation::ExecuteSelectOP(SelectOP& select_op, RelationSender sendRelation)
{
...
vector<Tuple> TupleOperation::GetUniqueOrderedTuples(vector<Tuple> Tuples, bool isUnique, ProjectOP project_op, string table_name, int index_memblock_end, vector<MyColumn> order_by)
{
		...
		if(isUnique)
		{
				...
				{
					string Name = TableProcessor::rand_table();
			...
			int memory_block_size = tuples_displayed.size()/tuples_displayed[0].getTuplesPerBlock();
				...
				for(myiterator = order_by.begin(); myiterator!=order_by.end(); myiterator++)
				{
					...
					if((*myiterator).tblName == NOT_A_TABLE)
						tble_name = TupleOperation::Fetch_Table_Name((*myiterator).colName);
