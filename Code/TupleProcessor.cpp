...
TupleProcessor::TupleProcessor(vector<string> fieldlist)
{
	
	field_names = fieldlist;
}

bool TupleProcessor::operator()(Tuple& tuple1, Tuple& tuple2)
{
	...
	if(schema_tuple1 != schema_tuple2)
		throw string("Error!! Tuple comparison failed - TupleProcessor::operator()");
...
bool InMemorySort(int start_index, int num_blocks, vector<string> field_names)
{
	...
	sort(mem_tuples.begin(), mem_tuples.end(), TupleProcessor(field_names));
	for(int index=start_index; index< num_blocks; index++)
	{
		mem.getBlock(index)->clear(); 
	}
...
Tuple search_min_tuple(vector<Tuple> tuples, vector<string> field_names)
{
	...
	myiterator = min_element(tuples.begin(), tuples.end(), TupleProcessor(field_names));
	return *myiterator;
}
